// components/AdminPuestos/CrearPuesto/CrearPuesto.jsx
...
import "./CrearPuesto.css";

export default function CrearPuesto({ isOpen, onClose, onPuestoCreado }) {
  const [formData, setFormData] = useState({
    puesto: "",
    descripcion: "",
    estatus_id: 1 // Por defecto activo
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(() => {
        // Notificar al componente padre que se creó un puesto
        if (onPuestoCreado) {
          onPuestoCreado();
        }
        handleCancelar();
  ...
  return (
    <div className="modal-overlay">
      <div className="modal-contenido crear-puesto-modal">
        ...
        <div className="modal-header">
          ...
          <p>Completa la información del nuevo puesto</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="formulario-modal">
          ...
          {exito && (
            <div className="success-message">
              <FaCheckCircle />
              <span>¡Puesto creado correctamente!</span>
            </div>
          )}
          ...
          <div className="modal-actions">
            ...
            <button
              ...
              disabled={enviando || exito}
            >
              {enviando ? "Creando..." : exito ? "¡Creado!" : <><FaSave /> Crear Puesto</>}
            </button>
          </div>
        </form>
      </div>
