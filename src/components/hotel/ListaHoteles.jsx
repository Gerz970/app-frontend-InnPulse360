...
export default function ListaHoteles() {
    // Inicializamos el estado directamente con los datos de prueba
    const [hoteles, setHoteles] = useState(mockHoteles); 
    const [busqueda, setBusqueda] = useState('');
    
    // Estos estados son innecesarios en la versión estática, pero los dejamos como useState(false/null) 
    // para evitar errores de linter si los usaste previamente:
    // NOTA: Para total corrección, simplemente NO uses las variables isLoading y error 
    // si las inicializas como 'const' como sugeriste antes.
    
    const navigate = useNavigate();

    // ----------------------------------------------------------------------
    // Se elimina el bloque de funciones asíncronas (cargarHoteles) y useEffect.
    // ----------------------------------------------------------------------
    ...
    const hotelesFiltrados = hoteles.filter(hotel =>
        ...
        (hotel.email_contacto && hotel.email_contacto.toLowerCase().includes(busqueda.toLowerCase()))
    );
    ...
    const handleEliminar = (hotelId) => {
        if (window.confirm(`[SIMULACIÓN] ¿Estás seguro de que quieres eliminar el hotel con ID ${hotelId}? Esta acción es solo local.`)) {
